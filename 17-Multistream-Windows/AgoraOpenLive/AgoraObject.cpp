    ...
    , m_bClickTest(FALSE)
{
	...
	m_localRenderMode = RENDER_MODE_FIT;
	m_remoteRenderMode = RENDER_MODE_FIT;
}
...
//set it for 17 communication
void CAgoraObject::SetLiveForCommParameters()
{
	AParameter apm(*m_lpAgoraEngine);
	apm->setParameters("{\"che.audio.live_for_comm\":true}");
	apm->setParameters("{\"che.video.moreFecSchemeEnable\":true}");

}
void CAgoraObject::SetLowBRSParameters(const int videoSolution)
{
	AParameter apm(*m_lpAgoraEngine);
	if (0 == videoSolution)
	{
		apm->setParameters("{\"che.video.lowBitRateStreamParameter\":{\"width\":320,\"height\":240,\"frameRate\":15,\"bitRate\":140}}");
	}
	else
	{
		apm->setParameters("{\"che.video.lowBitRateStreamParameter\":{\"width\":320,\"height\":180,\"frameRate\":15,\"bitRate\":140}}");
	}
}
BOOL CAgoraObject::EnableDualStream(BOOL bEnable)
{
	int nRet = 0;

	RtcEngineParameters rep(*m_lpAgoraEngine);

	nRet = rep.enableDualStreamMode(static_cast<bool>(bEnable));

	return nRet == 0 ? TRUE : FALSE;
}
BOOL CAgoraObject::SetRemoteVideoStreamType(UINT nUID, REMOTE_VIDEO_STREAM_TYPE nStreamType)
{
	int nRet = 0;

	RtcEngineParameters rep(*m_lpAgoraEngine);

	nRet = rep.setRemoteVideoStreamType(nUID, nStreamType);

	return nRet == 0 ? TRUE : FALSE;
}

BOOL CAgoraObject::SetVideoRender()
{
	int	nRet = 0;
...
void CAgoraObject::ClearAVSInfo()
{
	m_mapAVSInfo.RemoveAll();
}

void CAgoraObject::SetAVSInfo(UINT nUID, LPAVSTATUS_INFO lpAVSInfo)
{
	AVSTATUS_INFO avsInfo;

	ASSERT(nUID != 0xcccccccc);
	memset(&avsInfo, 0, sizeof(AVSTATUS_INFO));

	avsInfo.nUID = nUID;
	if (lpAVSInfo != NULL)
		memcpy(&avsInfo, lpAVSInfo, sizeof(AVSTATUS_INFO));

	m_mapAVSInfo.SetAt(nUID, avsInfo);
}

void CAgoraObject::RemoveAVSInfo(UINT nUID)
{
	m_mapAVSInfo.RemoveKey(nUID);
}

BOOL CAgoraObject::GetAVSInfo(UINT nUID, LPAVSTATUS_INFO lpAVSInfo)
{
	AVSTATUS_INFO avsInfo;

	if (!m_mapAVSInfo.Lookup(nUID, avsInfo))
		return FALSE;

	memcpy(lpAVSInfo, &avsInfo, sizeof(AVSTATUS_INFO));

	return TRUE;
}

BOOL CAgoraObject::GetAVSByIndex(int nIndex, LPAVSTATUS_INFO lpAVSInfo)
{
	int		nCounter = 0;

	if (nIndex < 0 || nIndex >= m_mapAVSInfo.GetCount())
		return FALSE;

	POSITION pos = m_mapAVSInfo.GetStartPosition();
	while (pos != NULL && nCounter < nIndex) {
		m_mapAVSInfo.GetNext(pos);
		nCounter++;
	}

	*lpAVSInfo = m_mapAVSInfo.GetValueAt(pos);

	return TRUE;
}
void CAgoraObject::GetRenderMode(RENDER_MODE_TYPE *nLocalRenderMode, RENDER_MODE_TYPE *nRemoteRenderMode)
{
	*nLocalRenderMode = m_localRenderMode;
	*nRemoteRenderMode = m_remoteRenderMode;
}

BOOL CAgoraObject::UpdateCaptureRegion(const LPRECT lpRect)
{
	ASSERT(m_lpAgoraEngine != NULL);
	ASSERT(lpRect != NULL);

	if (!m_bScreenCapture || lpRect == NULL)
		return FALSE;
	...
	ret = m_lpAgoraEngine->updateScreenCaptureRegion(&rect);
