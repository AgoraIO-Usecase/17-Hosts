...
BOOL CAgoraOpenLiveDlg::OnInitDialog()
{
	...
	CAgoraObject::GetAgoraObject()->SetLiveForCommParameters();
	CAgoraObject::GetAgoraObject()->EnableDualStream(TRUE);

	CAgoraObject::GetAgoraObject()->SetVideoRender();
...
{
	// TODO:  在此添加控件通知处理程序代码
    //if (!CAgoraObject::GetAgoraObject()->getClickTestFlag())//if runs on virtual machine, then shall add this???
...
LRESULT CAgoraOpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
    ...
    lpAgoraObject->SetLowBRSParameters(nVideoSolution);

    if (nVideoSolution == 0)
    {
        nVideoSolution = 40;
    }
    else if (nVideoSolution ==1)
    {
        nVideoSolution = 30;
    }
    else if (nVideoSolution == 2)
    {
        nVideoSolution = 39;
    }
